...
const ExcelFormulaBeautifier = {
    ...
    for (let i = 0; i < strs.length; i++) {
      if (E) {
        ...
        if (strs[i] === "") {
          ...
          E = !E;
          tempStr =
            tempStr + '"' + this.stringVarName + this.pureStrings.length + '"';
    ...
    for (let i = 0; i < words.length; i++) {
      ...
      if (word.endsWith("(")) {
        ...
        if (hit) {
          ...
          args.push(ExFunction[funcIndex].Args);
          argCounts.push(0);
        } else {
          // if not function
          leftBKs.push(0);
  ...
  getUsedFunctionArr: function () {
    // 将使用过的函数的信息显示出来
    const output = []; // {index:x,function:obj,rows:[]}
